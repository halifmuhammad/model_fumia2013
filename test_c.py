...
def test_c2_summary():
    ...
    fumia_a = pd.read_csv(dataset_ratio_a,index_col='x')
    fumia_p = pd.read_csv(dataset_ratio_p,index_col='x')
    fumia_q = pd.read_csv(dataset_ratio_q,index_col='x')
...
# dataset 
dataset_ratio_a = join(dirname(model_fumia2013.__file__), 
    'dataset-ratio-a.csv')
dataset_ratio_p = join(dirname(model_fumia2013.__file__), 
    'dataset-ratio-p.csv')
dataset_ratio_q = join(dirname(model_fumia2013.__file__), 
    'dataset-ratio-q.csv')

file_a2 = join(dirname(model_fumia2013.__file__),
    'output-a1-fumia-model-processed-weighted-sum.txt')

# output 
file_c1 = join(dirname(model_fumia2013.__file__), 
    'output-c1-results.json')
file_c2 = join(dirname(model_fumia2013.__file__), 
    'output-c2-results-summary.csv')
file_c2_plot_a = join(dirname(model_fumia2013.__file__), 
    'figure-c2-simul-results-summary')
file_c2_plot_b = join(dirname(model_fumia2013.__file__), 
